...
// 根据掩码值，如果为true就将返回值设置为value，否则保持原值不变
template <typename T>
...
// 根据掩码值，如果为true就将src的值复制到dest，否则保持原值不变
template <typename T>
...
// 从内存加载数据到向量，根据掩码值，如果为true就将src的值复制到dest，否则保持原值不变
template <typename T>
...
// 将向量数据存储到内存，根据掩码值，如果为true就将src的值复制到dest，否则保持内存中原值不变
template <typename T>
...
// 对向量进行水平加法操作，将相邻的两个元素相加得到的结果复制到相邻的两个元素所在位置上
template <typename T>
...
// 对向量vec按照交错方式重新排列，将偶数索引的元素放在前半部分，奇数索引的元素放在后半部分
template <typename T>
